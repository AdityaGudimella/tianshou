...
class BasePolicy(ABC, nn.Module):
    ...
    * :meth:`~tianshou.policy.BasePolicy.update`: the main interface for training, \
    ...
    def morl_learn(self, batch: api.TensorExperience) -> Dict[str, Any]:
    ...
    def morl_update(self, morl_memory: api.Memory = None) -> Dict[str, Any]:
        if morl_memory is None:
            raise NotImplementedError("morl_memory is not set")
        if not morl_memory:
            return {}
        batch = morl_memory.get_()
        self.updating = True
        batch, target_returns = self.morl_process_fn(batch)
        result = self.morl_learn(batch, target_returns)
        if self.lr_scheduler is not None:
            self.lr_scheduler.step()
        self.updating = False
        return result
    ...
    @staticmethod
    def morl_compute_nstep_return(
        batch: api.Experience,
        target_q_func: Callable[[api.TensorExperience], torch.Tensor],
        gamma: float = 0.99,
        n_step: int = 1,
    ) -> torch.Tensor:
        target_q_values = target_q_func(batch)
        return to_torch_as(
            batch.rewards
            + (gamma**n_step)
            * to_numpy(target_q_values.reshape(len(batch.rewards), -1))
